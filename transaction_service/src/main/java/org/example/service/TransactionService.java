...
public class TransactionService {
    ...
    public UserTransactionResponse createTransactionRequest(UserTransactionRequest request) throws JsonProcessingException {
        /*	This API will take the request, validate it.
			1. check if user exists or not
			2. check balance sufficiency
			3. Store all transaction details
			4. push message to Kafka
			5. Listener Wallet Service update the balance
			6. new message for transaction.(WHY Transaction? bcz we maintain one status -> Once we perform the 3rd status we mark the STATUS : PENDING,  and We will state the user that the status is pending, please wait. This is your transaction iD)
			7. POLL message from Transaction message.
			8. Update status
            9. Send new message to Kafka for Notification*/
    ...
    public String getTransactionDetailsById(String id) {
         Transaction transaction = transactionRepository.findById(id).get();
         if(transaction != null) {
             return transaction.toString();
         }
         return "Given an incorrect Transaction Id";
