...
public class KafkaConsumer {
    ...
    @KafkaListener(topics = TopicConstants.SUCCESSFUL_RECHARGE_TOPIC, groupId = "send_notification")
    public void sendRechargeNotification(ConsumerRecord receivedMessage) throws JsonProcessingException {
        RechargeResponseDto d = objectMapper.readValue(receivedMessage.value().toString(), RechargeResponseDto.class);
        log.info(String.format("Received Notification from Service: WALLET_RECHARGE with Message: %s", d.toString()));
        sendMail.sendRechargeMail(d);
    }
}
